...
def pay(request):
    MAINPAYLOAD = {
        ...
        "amount": 10000,
        "redirectUrl": "http://0.0.0.0:8000/phonepay/return-to-me/",
        "redirectMode": "POST",
        "callbackUrl": "http://0.0.0.0:8000/phonepay/return-to-me/",
    ...
    if 'data' in responseData and 'instrumentResponse' in responseData['data'] and 'redirectInfo' in responseData['data']['instrumentResponse']:
        return redirect(responseData['data']['instrumentResponse']['redirectInfo']['url'])
    else:
        # Handle the error appropriately, e.g., log the error, return an error message, etc.
        return HttpResponse("Error: Invalid response from PhonePe API", status=500)
